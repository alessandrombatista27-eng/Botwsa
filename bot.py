...
BANNER_URL = (
    "https://cdn.discordapp.com/attachments/1483577134206160977"
    "/1484355094630895626/content.png"
    "?ex=69bdece8&is=69bc9b68"
    "&hm=507e43d5d3da932f3f7f6eef09d99f8380faf7e9f5d284a902492cbac5612719&"
)
...
CARGOS_TIMES = [
    1484546866233344111, 1484546933115715636, 1484546961184129024,
    1484546994797023313, 1484547020260905130, 1484547051889885356,
    1484547078624514131, 1484547111264456797,
]
...
def _accent(color: int) -> discord.ui.Container:
    """Retorna um Container vazio só para definir a cor lateral (accent)."""
    return discord.ui.Container(accent_colour=discord.Colour(color))
...
def make_container(
    *lines: str,
    ...
    """
    Constrói um Container V2 com:
      - TextDisplay para cada linha
      - Thumbnail opcional (MediaGallery de 1 item, float não existe → usamos Section)
      - Image opcional no final
      - Separator + footer opcional
    """
    children: list = []

    # Corpo principal (Section com thumbnail ou só texto)
    text_block = discord.ui.TextDisplay("\n".join(lines))
    ...
    # Componentes extras (botões, selects passados como filhos)
    if components:
    ...
    if image_url:
        ...
        children.append(
            discord.ui.MediaGallery(
                items=[discord.ui.MediaGalleryItem(
...
async def on_message(message: discord.Message):
    ...
    if LINK_BLOQUEADO.search(message.content):
        ...
        container = make_container(
            f"🚫 {message.author.mention} links de convite do Roblox não são "
            "permitidos neste servidor!",
            color=0xFF4444,
        )
...
def gerar_contract_id(signee_id, contractor_id):
    ...
    return f"WSA{signee_id}{contractor_id}{ts}"


# ══════════════════════════════════════════════
# CONTAINERS V2 — CONTRATOS
# ══════════════════════════════════════════════

def container_contrato_pendente(c: dict) -> discord.ui.Container:
    ...
    corpo = (
        "## 📋 Proposta de Contrato — WSA League\n\n"
        f"**Contratado:** <@{c['signee_id']}> — {c['signee_name']}\n"
        f"**Contratante:** <@{c['contractor_id']}> — {c['contractor_name']}\n"
        f"**Contract ID:** `{c['contract_id']}`\n"
        f"**Time:** {c.get('team', '—')}\n"
        f"**Posição:** {c.get('position', '—')}\n"
        f"**Cargo:** {c.get('role', '—')}\n"
    )
    footer = f"WSA League • Emitido: {issued} • Expira: {expires}"
    return make_container(corpo, color=0xFF6600, thumbnail_url=LOGO_URL, footer=footer)


def container_aceito(c: dict) -> discord.ui.Container:
    corpo = (
        "## ✅ Contrato Aceito\n\n"
        f"<@{c['signee_id']}> aceitou o contrato e agora faz parte do time "
        f"**{c.get('team', '—')}**!\n\n"
        f"**Contratado:** <@{c['signee_id']}> — {c['signee_name']}\n"
        f"**Contratante:** <@{c['contractor_id']}> — {c['contractor_name']}\n"
        f"**Contract ID:** `{c['contract_id']}`"
    )
    return make_container(corpo, color=0x2ECC71, thumbnail_url=LOGO_URL, footer="WSA League")


def container_recusado(c: dict) -> discord.ui.Container:
    corpo = (
        "## ❌ Contrato Recusado\n\n"
        f"<@{c['signee_id']}> recusou a proposta de contrato.\n\n"
        f"**Contratado:** <@{c['signee_id']}> — {c['signee_name']}\n"
        f"**Contratante:** <@{c['contractor_id']}> — {c['contractor_name']}\n"
        f"**Contract ID:** `{c['contract_id']}`"
    )
    return make_container(corpo, color=0x95A5A6, footer="WSA League")


def container_expirado(c: dict) -> discord.ui.Container:
    corpo = (
        "## ⏰ Contrato Expirado\n\n"
    ...
    return make_container(corpo, color=0x992D22, footer="WSA League")
...
class ContratoView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="btn_aceitar_contrato",
    )
    async def aceitar(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.signee_id:
            await interaction.response.send_message(
                "❌ Apenas o contratado pode aceitar este contrato.", ephemeral=True
            )
            return
        ...
        await interaction.message.edit(
            components=[container_aceito(c)],
            view=None,
        )
        conf = make_container(
            f"🎉 {interaction.user.mention} aceitou o contrato e foi contratado "
            f"para o time **{c.get('team', '—')}**!",
            color=0x2ECC71,
        )
        await interaction.response.send_message(
            components=[conf],
    ...
    @discord.ui.button(
        ...
        custom_id="btn_recusar_contrato",
    )
    async def recusar(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.signee_id:
            await interaction.response.send_message(
                "❌ Apenas o contratado pode recusar este contrato.", ephemeral=True
            )
            return
        ...
        await interaction.message.edit(
            components=[container_recusado(c)],
            view=None,
        )
        conf = make_container(
            f"❌ {interaction.user.mention} recusou o contrato.",
            color=0x95A5A6,
        )
        await interaction.response.send_message(
            components=[conf],
...
async def checar_contratos_expirados():
    ...
    now = time.time()
    expirados = [
        cid for cid, c in db["contracts"].items()
    ...
    for cid in expirados:
        ...
        try:
            ...
            if channel and c.get("message_id"):
                ...
                await msg.edit(components=[container_expirado(c)], view=None)
...
class AvaliacaoModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        cont  = make_container(corpo, color=cor, footer=f"WSA League • Avaliações de Tickets • {ts}")
...
class AvaliacaoView(discord.ui.View):
    ...
    async def _avaliar(self, interaction: discord.Interaction, nota: int):
        await interaction.response.send_modal(
            AvaliacaoModal(nota, self.dono_id, self.canal_nome)
        )
        self.stop()

    @discord.ui.button(label="⭐ 1", style=discord.ButtonStyle.danger,    custom_id="aval_1")
    async def aval1(self, i: discord.Interaction, b: discord.ui.Button): await self._avaliar(i, 1)

    @discord.ui.button(label="⭐ 2", style=discord.ButtonStyle.danger,    custom_id="aval_2")
    async def aval2(self, i: discord.Interaction, b: discord.ui.Button): await self._avaliar(i, 2)

    @discord.ui.button(label="⭐ 3", style=discord.ButtonStyle.secondary, custom_id="aval_3")
    async def aval3(self, i: discord.Interaction, b: discord.ui.Button): await self._avaliar(i, 3)

    @discord.ui.button(label="⭐ 4", style=discord.ButtonStyle.success,   custom_id="aval_4")
    async def aval4(self, i: discord.Interaction, b: discord.ui.Button): await self._avaliar(i, 4)

    @discord.ui.button(label="⭐ 5", style=discord.ButtonStyle.success,   custom_id="aval_5")
    async def aval5(self, i: discord.Interaction, b: discord.ui.Button): await self._avaliar(i, 5)


def pegar_dono_id(topic: str) -> int | None:
...
class TicketView(discord.ui.View):
            ...
            try:
                ...
                if membro:
                    ...
                    dm_cont = make_container(
                        ...
                        footer="WSA League • Ticket System",
                    )
                    await membro.send(
                        components=[dm_cont],
                        view=AvaliacaoView(dono_id, canal_nome, interaction.guild.id),
                        flags=discord.MessageFlags(is_components_v2=True),
                    )
            except Exception:
                pass
    ...
    async def painel_membro(self, interaction: discord.Interaction, button: discord.ui.Button):
        if any(r.id in CARGOS_SUPORTE for r in interaction.user.roles):
            await interaction.response.send_message(
                "❌ Este painel é exclusivo para membros!", ephemeral=True
            )
            return
        cont = make_container(
            "## 👤 Painel Membro\n\nUse as opções abaixo para interagir com a staff:",
            color=0x5865F2,
            footer="WSA — Painel Membro",
        )
        await interaction.response.send_message(
            components=[cont],
            view=PainelMembroView(),
            ephemeral=True,
            flags=discord.MessageFlags(is_components_v2=True),
        )
    ...
    async def painel_staff(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not (eh_suporte or interaction.user.guild_permissions.administrator):
            await interaction.response.send_message(
                "❌ Este painel é exclusivo para a staff!", ephemeral=True
            )
            return
        cont = make_container(
            "## 👮 Painel Staff\n\nUse as opções abaixo para interagir com o membro:",
            color=0xFF6600,
            footer="WSA — Painel Staff",
        )
        await interaction.response.send_message(
            components=[cont],
            view=PainelStaffView(),
            ephemeral=True,
            flags=discord.MessageFlags(is_components_v2=True),
        )
...
class PainelMembroSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(
                ...
                description="Envia um ping para quem está atendendo o ticket.",
                emoji="🔔",
        ...
        super().__init__(
            ...
            options=options,
            custom_id="painel_membro_select",
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if canal_id in tickets_assumidos:
            ...
            await interaction.response.send_message(
                f"🔔 {interaction.user.mention} está chamando o responsável pelo ticket!\n"
                f"<@{resp_id}>, o membro precisa de você!"
            )
        else:
            mencoes = " ".join(
                f"<@&{cid}>" for cid in CARGOS_SUPORTE
                if interaction.guild.get_role(cid)
            )
            await interaction.response.send_message(
                f"🔔 {interaction.user.mention} está chamando a staff!\n{mencoes}"
            )
...
class PainelStaffSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(
                ...
                description="Envia um ping para o membro no canal.",
                emoji="📣",
                value="notificar_membro",
            ),
            discord.SelectOption(
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "notificar_membro":
            ...
            await interaction.response.send_message(
                f"📣 {interaction.user.mention} está chamando o {mencao}!\n"
                f"{mencao}, a staff precisa de você no ticket!"
            )
            ...
            if canal_id in tickets_assumidos:
                resp_id = tickets_assumidos[canal_id]
                msg = (
                    "⚠️ Você já é o responsável por este ticket!"
                    if resp_id == interaction.user.id
                    else f"⚠️ Este ticket já foi assumido por <@{resp_id}>!"
                )
            ...
            await interaction.response.send_message(
                f"✋ {interaction.user.mention} assumiu este ticket e irá te atender!\n"
                f"{mencao} seu ticket agora está sendo atendido por {interaction.user.mention}."
            )
...
class TicketSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        regras = make_container(
            ...
            footer="Só você pode ver esta mensagem",
        )
        await interaction.response.send_message(
            components=[regras],
            view=ConfirmarTicketView(categoria_key),
            ephemeral=True,
            flags=discord.MessageFlags(is_components_v2=True),
        )
...
async def criar_canal_ticket(interaction: discord.Interaction, categoria_key: str):
    ...
    if canal_existente:
        await interaction.followup.send(
            f"❌ Você já tem um ticket aberto em {canal_existente.mention}!", ephemeral=True
        )
        return
    ...
    canal_ticket = await guild.create_text_channel(
        ...
        topic=(
            ...
            f"Categoria: {cat['label']} | ID: {interaction.user.id}"
        ),
    )
    ...
    cont = make_container(
        ...
        color=0xFF6600,
        thumbnail_url=LOGO_URL,
        footer=f"WSA League • Suporte • {ts}",
    )
    await canal_ticket.send(
        content=f"{interaction.user.mention} {mencoes}",
        components=[cont],
        view=TicketView(),
        flags=discord.MessageFlags(is_components_v2=True),
    )
    await interaction.followup.send(
        f"✅ Ticket aberto em {canal_ticket.mention}!", ephemeral=True
    )
...
@app_commands.describe(
    ...
    canal="Canal onde o aviso será enviado (padrão: canal atual)",
)
async def aviso(
    ...
    cont = make_container(
        ...
        color=0xFF6600,
        thumbnail_url=LOGO_URL,
        footer=f"Aviso emitido por {interaction.user.display_name} • {ts}",
    )
    ...
    await interaction.followup.send(
        f"✅ Aviso enviado com sucesso em {canal_alvo.mention}!", ephemeral=True
    )
...
async def ticket_cmd(
    ...
    if not (
        ...
        await interaction.response.send_message(
            "❌ Você não tem permissão para configurar tickets!", ephemeral=True
        )
        return
    ...
    cont = make_container(
        ...
        "fora esse horário a equipe não tem obrigação de respondê-lo.\n\n"
        "A equipe WSA agradece.",
        color=0xFF6600,
        thumbnail_url=LOGO_URL,
        footer="WSA League • Suporte",
    )
    await canal_alvo.send(
        components=[cont],
    ...
    await interaction.followup.send(
        f"✅ Painel de ticket enviado em {canal_alvo.mention}!", ephemeral=True
    )
...
async def contratar(
    ...
    if not (manager_role and manager_role in interaction.user.roles):
        await interaction.response.send_message(
            "❌ Você precisa ter o cargo de Manager para contratar membros!", ephemeral=True
        )
        return
    if membro.bot:
    ...
    if cargo.id not in CARGOS_TIMES:
        cargos_mencoes = "\n".join(
            f"<@&{cid}>" for cid in CARGOS_TIMES if interaction.guild.get_role(cid)
        )
        await interaction.response.send_message(
            f"❌ Cargo inválido! Cargos permitidos:\n{cargos_mencoes}", ephemeral=True
        )
        return
    ...
    cont  = container_contrato_pendente(c)
    ...
    if not canal_contratos:
        await interaction.response.send_message(
            "❌ Canal de contratos não encontrado.", ephemeral=True
        )
        return

    notif = (
        f"{membro.mention}, você recebeu uma proposta de contrato "
...
async def contratos_ativos(interaction: discord.Interaction):
    ...
    linhas = ["## ⏳ Contratos Pendentes — WSA League\n"]
    for c in pending:
        expires = datetime.datetime.fromtimestamp(c["expires_at"]).strftime("%d/%m %H:%M")
        linhas.append(
            f"**🔸 {c.get('team','—')} — {c['position']}**\n"
            f"<@{c['signee_id']}> ← <@{c['contractor_id']}> | Expira: {expires}\n"
        )

    cont = make_container("\n".join(linhas), color=0xFF6600)
    await interaction.response.send_message(
        components=[cont],
        ephemeral=True,
        flags=discord.MessageFlags(is_components_v2=True),
...
async def historico_contratos(
    ...
    if membro:
        history = [
            c for c in history
            if c["signee_id"] == membro.id or c["contractor_id"] == membro.id
        ]
    if not history:
        await interaction.response.send_message("📭 Nenhum contrato encontrado.", ephemeral=True)
        return

    emoji_map = {"accepted": "✅", "declined": "❌", "expired": "⏰", "cancelled": "🗑️"}
    linhas    = ["## 📋 Histórico de Contratos — WSA League\n"]
    for c in list(reversed(history))[:10]:
        emoji = emoji_map.get(c["status"], "❓")
        linhas.append(
            f"{emoji} **{c.get('team','—')} — {c.get('position','—')}**\n"
            f"<@{c['signee_id']}> ← <@{c['contractor_id']}> | {c['status'].upper()}\n"
        )
...
async def cancelar_contrato(interaction: discord.Interaction, contract_id: str):
    ...
    if not (is_contractor or is_manager):
        await interaction.response.send_message(
            "❌ Apenas quem enviou o contrato pode cancelá-lo.", ephemeral=True
        )
        return
    ...
    save_db(db)
    await interaction.response.send_message(
        f"🗑️ Contrato `{contract_id}` cancelado com sucesso.", ephemeral=True
    )
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.watching, name="WSA League"
        )
    )
